...
module Temporal::TestPlainMonthDay
  class Calendar < Minitest::Test
    def test_show_explicit_year_when_calendar_name_is_always
      subject = Temporal::PlainMonthDay.new(10, 31, calendar_id: :iso8601, year: 2019)
    ...
    def test_show_implicit_year_when_calendar_name_is_always
      subject = Temporal::PlainMonthDay.new(10, 31, calendar_id: :iso8601)

      assert_equal(
        "1972-10-31[u-ca=iso8601]",
        subject.to_s(calendar_name: :always),
        "emit fallback year if calendar_name = 'always' (static from)"
      )
    end
  end
end
